...
in
{
  ...
  ++ lib.optionals (rykCfg.niri.enable) [ inputs.dankMaterialShell.homeModules.niri ];

  config =
    let
      screenshotEditor = dankCfg.screenshotBackend;
    in
    lib.mkIf rykCfg.dankMaterialShell.enable {
      ...
      programs.niri.settings = lib.mkIf (rykCfg.niri.enable && rykCfg.niri.bar == "dankMaterialShell") {
