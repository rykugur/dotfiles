...
| `git.nu` aliases | `gits` → `gits`; `gll` → `gll`; `gbn` → `gbn`; `git head` → `git.head` (`gll --oneline -1`); `git.back` → `git.back`; `git.branch` → `git.branch`; `git.lastcommit` → `git.lastcommit`; `git.track` → `git.track`; `git.tree` → `git.tree` (`git log --graph --pretty=format:'%Cgreen[%h]%Creset %C(white)%d%Creset %Cblue%ad by %an%Creset: %s' --date=relative`); `jedi` → `jedi` |
...
| `k8s.nu` | `kubectl` → `kubectl`; `kubemerge` → `kubemerge`; `talosmerge` → `talosmerge`; `keit` → `kubectl exec -it`; `shlink create --slug SLUG --url URL` → `shlink-create --slug SLUG --url URL` **[exact]**; `shlink list` → `kubectl --namespace shlink exec -it deployments/shlink -- bin/cli short-url:list`; `hf` → `helmfile`; `fgk` → `flux get kustomization`; `mk` → `minikube`; `sops kaf` → `sops-kaf`; `k8s base64` → `k8s-base64` |
| `linux.nu` aliases 1 | `awk1` → `awk1`; `awk2` → `awk2`; `awk3` → `awk3`; `awk4` → `awk4`; `awk5` → `awk5`; `awk6` → `awk6`; `awk7` → `awk7`; `awk8` → `awk8`; `awk9` → `awk9`; `db` → `distrobox`; `cat` → `cat` (`bat`) **[exact]**; `dfh` → `dfh` (`duf`) **[exact]**; `duh` → `du -h`; `e` → `$EDITOR`; `getmyip` → `getmyip` (`curl -L ifconfig.me`) **[exact]**; `grep` → `grep`; `ll` → `ll` (`command ls -al`); `murder` → `murder`; `nv` → `nv`; `v` → `v`; `vi` → `vi`; `pingtest` → `pingtest`; `replace.newlines` → `replace.newlines`; `tmat` → `tmat` (`sesh connect (sesh list -t | fzf)`) **[exact]**; `top` → `top` (`btop`) **[exact]**; `whatthecommit` → `whatthecommit`; `ytdl` → `ytdl` |
| `linux.nu` function | `stay-awake` → `stay-awake` |
| `misc.nu` functions 1 | `dots --edit/-e --local/-l` → `dots --edit/-e --local/-l` (local mode creates `$FISH_LOCAL_CONFIG_FILE`, which defaults to `$HOME/.local/fish/config.fish` independently of the inherited Nushell `$LOCAL_CONFIG_FILE`, enters its directory, and edits that file); `is-os OS` → `is-os OS` (compares `uname`; `macos` and `darwin` both match Darwin); `is-darwin` → `is-darwin`; `is-macos` → `is-macos`; `is-linux` → `is-linux`; `cmd.copy` → `cmd-copy`; `cmd.paste` → `cmd-paste`; `replace-multiline` → `replace-multiline`; `paste-multiline-nu` → `paste-multiline` **[partial: executes its clipboard content with `fish -c`, so it accepts Fish syntax rather than Nushell syntax]**; `curl multiline` → `curl-multiline` **[partial: safely tokenizes literal shell-style words, quotes, and escapes, but does not evaluate Nushell expressions such as `$env.API_URL`, command substitutions, or other Nu syntax]**; `edit-multiline` → `edit-multiline` |
| `misc.nu` functions 2 | `ghostty fix terminfo HOST` → `ghostty-fix-terminfo HOST` **[exact]**; `1password copy-ssh-pub-key HOST` → `1password-copy-ssh-pub-key HOST` **[exact]**; `1password get-ssh-pub-key` → `op-ssh-public-key`; `1password get-private-age-key` → `sops-age-private-key`; `1password get-public-age-key` → `sops-age-public-key`; `proxmox install helix HOST` → `proxmox-install-helix HOST` **[exact]**; `mo2installer` → `cd $HOME/gits/modorganizer2-linux-installer` |
| `misc.nu` abbreviations 1 | `adb.reverse` → `adb.reverse`; `adb.start` → `adb.start`; `adb.reset-perms` → `adb.reset-perms`; `.gw` → `.gw`; `agb` → `agb`; `agbt` → `agbt`; `fish.profile` → `fish.profile`; `pyhttp` → `pyhttp`; `pyjson` → `pyjson`; `cwd` → `cwd`; `gri` → `gri`; `grin` → `grin`; `grine` → `grine`; `pwdc` → `pwdc`; `ssh.forcePass` → `ssh.forcePass`; `taill` → `taill`; `pn` → `pn`; `psw` → `psw FIELD OPERATOR VALUE` **[partial: filters padded `ps -eo pid=,comm=,pcpu=,pmem=,args=` records by `pid`, `name`, `cpu`, `mem`, or `command` with `=`, `!=`, numeric comparison, or `=~`; quote `<`, `>`, `<=`, and `>=` because Fish parses them as redirections; output remains records rather than a Nushell table]**; `pagi PROCESS_NAME_REGEX` → `pagi PROCESS_NAME_REGEX` **[partial: applies a Fish regex to the structured `comm` process-name field, but emits `ps -eo` records rather than a Nushell table]** |
...
| `nix.nu` functions | `rbld` → `rbld`; `rbld switch` → `rbld-switch`; `rbld boot` → `rbld-boot`; `nd SHELL` → `nd SHELL` (`nix develop $DOTFILES_DIR#SHELL`); `nix get-hash` → `nix-get-hash`; `shash URL REV` → `shash URL REV` (`nix-prefetch-git --url URL --rev REV`); `nr.` → `nr.` (`nix repl --expr "builtins.getFlake \"$PWD\""`); `nrd` → `nrd`; `nrn` → `nrn`; `nrf` → `nrf`; `nrun` → `nrun`; `mkenvrc` → `mkenvrc`; `mkflake` → `mkflake`; `mkflake electrobun` → `mkflake-electrobun` |
...
| `zellij.nu` | `zellij exists SESSION` → `zellij-exists SESSION` (Fish regex matching retains Nu substring/regex semantics); `zellij create-or-attach SESSION [--layout PATH]` → `zellij-create-or-attach SESSION [--layout PATH]`; `zellij fzf` → `zellij-fzf`; `zellij delete-all-sessions` → `zellij-delete-all-sessions`; `zellij murder-all-sessions` → `zellij-murder-all-sessions` |
