...
| `misc.nu` abbreviations 1 | `adb.reverse` → `adb.reverse`; `adb.start` → `adb.start`; `adb.reset-perms` → `adb.reset-perms`; `.gw` → `.gw`; `agb` → `agb`; `agbt` → `agbt`; `fish.profile` → `fish.profile`; `pyhttp` → `pyhttp`; `pyjson` → `pyjson`; `cwd` → `cwd`; `gri` → `gri`; `grin` → `grin`; `grine` → `grine`; `pwdc` → `pwdc`; `ssh.forcePass` → `ssh.forcePass`; `taill` → `taill`; `pn` → `pn`; `psw` → `psw FIELD OPERATOR VALUE` **[partial: Fish-native field filtering for `pid`, `name`, `cpu`, `mem`, and `command` with `=`, `!=`, numeric comparison, or `=~`; quote `<`, `>`, `<=`, and `>=` because Fish parses them as redirections, and output is `ps -eo` records rather than Nushell tables]**; `pagi` → `pagi` |
