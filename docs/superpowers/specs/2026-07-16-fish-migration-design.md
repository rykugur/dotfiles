...
| `misc.nu` functions 1 | `dots --edit/-e --local/-l` → `dots --edit/-e --local/-l` (local mode creates `$LOCAL_CONFIG_FILE`, enters its directory, and edits that file); `is-os OS` → `is-os OS` (compares `uname`; `macos` and `darwin` both match Darwin); `is-darwin` → `is-darwin`; `is-macos` → `is-macos`; `is-linux` → `is-linux`; `cmd.copy` → `cmd-copy`; `cmd.paste` → `cmd-paste`; `replace-multiline` → `replace-multiline`; `paste-multiline-nu` → `paste-multiline`; `curl multiline` → `curl-multiline` **[partial: safely tokenizes literal shell-style words, quotes, and escapes, but does not evaluate Nushell expressions such as `$env.API_URL`, command substitutions, or other Nu syntax]**; `edit-multiline` → `edit-multiline` |
