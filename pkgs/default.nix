...
// (
  if sptSrc != null then
    (
          ...
          pkgs.stdenv.mkDerivation {
            ...
            installPhase = ''
              ...
              wrapProgram $out/bin/${name} \
                --prefix PATH : ${
                  lib.makeBinPath (builtins.filter (x: x != null) (sptRuntimeDeps ++ [ spt7zzs ]))
                }
            '';
            meta = {
              inherit description;
      ...
      {
        spt-additions =
          mkSptTool "spt-additions" "scripts/spt-additions"
            "SPT additions CLI installer & manager script (from SPT-Linux-Guide)";
        ...
        spt-launcher =
          let
            drv = pkgs.writeShellScriptBin "spt-launcher" ''
              ...
              exec ${
                if pkgs ? umu-launcher then "${pkgs.umu-launcher}/bin/umu-run" else "umu-run"
              } "$LAUNCHER" "$@"
            '';
          in
          drv.overrideAttrs (old: {
