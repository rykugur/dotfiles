...
- Host configs live in `modules/hosts/<name>/` with `_`-prefixed files excluded
  from import-tree
- AI agent definitions are shared across tools via `modules/ai/_agents.nix`

### Secrets

SOPS with age encryption. Keys in `.sops.yaml`, per-host `secrets.yaml` files in
`modules/hosts/<name>/`.

## TODO

- [ ] Remove passing username/hostname around — create option/config for it
- [ ] Add bambu 3d printing module
