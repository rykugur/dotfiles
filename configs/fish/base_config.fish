...
set -x fish_user_abbreviations 'ga=git add' 'gc=git commit' 'gco=git checkout' 'gd=git diff' 'gds=git diff --staged' 'gf=git fetch' 'gg=git grep' 'gp=git pull' 'supac=sudo pacman' 'supacs=sudo pacman -S' 'pac=pacman' 'sc=systemctl' 'ssc=sudo systemctl' 'sscr=sudo systemctl restart'
