...
set -x fish_user_abbreviations '..=cd ..' '...=cd ../..' '....=cd ../../..' 'ga=git add' 'gc=git commit' 'gca=git commit --amend' 'gco=git checkout' 'gd=git diff' 'gds=git diff --staged' 'gf=git fetch' 'gfp=git fetch -p' 'gg=git grep -n' geg='git grep -E' 'gp=git pull' 'gs=git stash' 'pagi=ps aux | grep -i' 'supac=sudo pacman' 'supacs=sudo pacman -S' 'pac=pacman' pacss='pacman -Ss' 'sc=systemctl' 'ssc=sudo systemctl' 'sscr=sudo systemctl restart'
